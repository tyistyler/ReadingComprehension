...
def convert_examples_to_features(
    ...
    for (ex_index, example) in tqdm.tqdm(enumerate(examples), desc="convert examples to features"):
        ...
        choices_features = [
          ...
          (input_ids3, attention_mask3, token_type_ids3),
          (input_ids4, attention_mask4, token_type_ids4)
        ]
        """
        choices_features = []
        ...
        for ending_idx, (context, ending) in enumerate(zip(example.contexts, example.endings)):
            text_a = context
            if example.question.find("_") != -1:          # 找不到"_"时
                # this is for cloze question
                """for instance
                The consultant responds to the lawmaker's argument by __.
            ...
            # encode_plus-->把text_a和text_b进行拼接，并转换为id，中间用[SEP]分割
